...
def generate_ROM():
    ...
    try:
        ...
        settings = RandomizerData(seed_int, get_difficulty(), get_goal(), get_logic(), statues.get(), get_enemizer(), get_start_location(),
            firebird.get(), ohko.get(), red_jewel_madness.get(), glitches.get(), boss_shuffle.get(), open_mode.get(), mute_music.get())
...
tkinter.Label(mainframe, text="Mute Music").grid(row=14, column=0, sticky=tkinter.W)
...
mute_music_checkbox = tkinter.Checkbutton(mainframe, variable=mute_music, onvalue=1, offvalue=0).grid(row=14, column=1)
